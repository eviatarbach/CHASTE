...
void BoundaryTrackingModifier<DIM>::UpdateCellData(AbstractCellPopulation<DIM,DIM>& rCellPopulation)
{
    ...
    else if (dynamic_cast<NodeBasedCellPopulation<DIM>*>(&rCellPopulation))
    {
        ...
        NodesOnlyMesh<DIM>* mpNodesOnlyMesh = &pCellPopulation->rGetMesh();
                ...
                ++cell_iter)
        {
            ...
            Node<DIM>* p_node = pCellPopulation->GetNode(node_index);
            ...
            c_vector<double, DIM> node_i_location = p_node->rGetLocation();

            // Get the set of node indices corresponding to this cell's neighbours
            std::set<unsigned> neighbouring_node_indices = pCellPopulation->GetNeighbouringNodeIndices(node_index);
            ...
            for (std::set<unsigned>::iterator iter = neighbouring_node_indices.begin();
                    ...
                    ++iter)
            {
                Node<DIM>* p_node_j = pCellPopulation->GetNode(*iter);

...
template<unsigned DIM>
void NagaiHondaContractilityForce<DIM>::AddForceContribution(AbstractCellPopulation<DIM>& rCellPopulation)
{
    // Throw an exception message if not using a VertexBasedCellPopulation
    if (dynamic_cast<VertexBasedCellPopulation<DIM>*>(&rCellPopulation) == NULL)
    {
        EXCEPTION("NagaiHondaContractilityForce is to be used with a VertexBasedCellPopulation only");
    }
    ...
    std::vector<double> contractilities(num_elements);
         ...
         ++elem_iter)
    {
        ...
        {
            EXCEPTION("You need to add an AbstractTargetAreaModifier to the simulation in order to use NagaiHondaContractilityForce");
        }
        try
        {
            contractilities[elem_index] = p_cell_population->GetCellUsingLocationIndex(elem_index)->GetCellData()->GetItem("contractility");
        }
        catch (Exception&)
        {
            contractilities[elem_index] = 0.0;
        }
    }
    ...
    for (unsigned node_index=0; node_index<num_nodes; node_index++)
    {
        ...
        c_vector<double, DIM> contractility_contribution = zero_vector<double>(DIM);
...
template<unsigned DIM>
double NagaiHondaContractilityForce<DIM>::GetAdhesionParameter(Node<DIM>* pNodeA, Node<DIM>* pNodeB, VertexBasedCellPopulation<DIM>& rVertexCellPopulation)
{
...
template<unsigned DIM>
double NagaiHondaContractilityForce<DIM>::GetNagaiHondaDeformationEnergyParameter()
{
    return mNagaiHondaDeformationEnergyParameter;
}

template<unsigned DIM>
double NagaiHondaContractilityForce<DIM>::GetNagaiHondaMembraneSurfaceEnergyParameter()
{
    return mNagaiHondaMembraneSurfaceEnergyParameter;
}

template<unsigned DIM>
double NagaiHondaContractilityForce<DIM>::GetNagaiHondaCellCellAdhesionEnergyParameter()
{
    return mNagaiHondaCellCellAdhesionEnergyParameter;
}

template<unsigned DIM>
double NagaiHondaContractilityForce<DIM>::GetNagaiHondaCellBoundaryAdhesionEnergyParameter()
{
    return mNagaiHondaCellBoundaryAdhesionEnergyParameter;
}

template<unsigned DIM>
void NagaiHondaContractilityForce<DIM>::SetNagaiHondaDeformationEnergyParameter(double deformationEnergyParameter)
{
    mNagaiHondaDeformationEnergyParameter = deformationEnergyParameter;
}

template<unsigned DIM>
void NagaiHondaContractilityForce<DIM>::SetNagaiHondaMembraneSurfaceEnergyParameter(double membraneSurfaceEnergyParameter)
{
    mNagaiHondaMembraneSurfaceEnergyParameter = membraneSurfaceEnergyParameter;
}

template<unsigned DIM>
void NagaiHondaContractilityForce<DIM>::SetNagaiHondaCellCellAdhesionEnergyParameter(double cellCellAdhesionEnergyParameter)
{
    mNagaiHondaCellCellAdhesionEnergyParameter = cellCellAdhesionEnergyParameter;
}

template<unsigned DIM>
void NagaiHondaContractilityForce<DIM>::SetNagaiHondaCellBoundaryAdhesionEnergyParameter(double cellBoundaryAdhesionEnergyParameter)
{
...
template<unsigned DIM>
void NagaiHondaContractilityForce<DIM>::OutputForceParameters(out_stream& rParamsFile)
{
...
template class NagaiHondaContractilityForce<1>;
template class NagaiHondaContractilityForce<2>;
template class NagaiHondaContractilityForce<3>;
